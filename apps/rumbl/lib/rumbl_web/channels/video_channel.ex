defmodule RumblWeb.VideoChannel do
  ...
  defp broadcast_annotation(socket, annotation) do
    ...
    rendered_ann = Phoenix.View.render(RumblWeb.AnnotationView, "annotation.json", %{annotation: annotation})
  ...
  defp compute_additional_info(ann, socket) do
    for result <- InfoSys.compute(ann.body, limit: 1, timeout: 10_000) do
      ...
      info_changeset = 
        Repo.get_by!(Rumbl.Accounts.User, username: result.backend)
        |> build_assoc(:annotations, video_id: ann.video_id)
        |> Rumbl.Contents.Annotation.changeset(attrs)
